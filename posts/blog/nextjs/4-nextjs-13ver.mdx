...
저는 Next.js로 SSG 정적 블로그를 개발한 경험만 있어서 위의 상황을 마주하진 못했지만
CSR 방식으로 개발할 때는 이러한 설정이 다소 번거로울 수도 있겠다는 생각이 드는 것 같습니다.
(리팩토링 할 때는 더더욱..)

혹시 CSR 방식을 13 버전으로 리팩토링 하셨거나 클라이언트 컴포넌트를 사용하고 있는 분들이 계시다면
어떻게 활용하고 계신지, 파일마다 설정을 해야해서 번거롭진 않으셨는지 의견 여쭙고 싶습니다..!
